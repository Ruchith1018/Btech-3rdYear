...
def facereg():
    ...
    if request.method == "POST":
        ...
        new_image_path = f'./static/face/unknown/{user_id}.jpg'
        new_image_handle = open(new_image_path, 'wb')
        new_image_handle.write(decoded_data)
        new_image_handle.close()

        try:
            image_of_user = face_recognition.load_image_file(f'./static/face/{user_id}.jpg')
            ...
            # Compare faces with a threshold value
            threshold = 0.5  # Adjust this threshold value as needed
            results = face_recognition.face_distance([user_face_encoding], unknown_face_encodings[0])
            print(results)
            if results[0] <= threshold:
                # Use the ID of the authenticated user
